...
export default {
  ...
  mounted() {
     this.switchBtn = this.$store.state.getInfo.customizedData.yearlysales;
     this.sqlString = this.$store.state.getInfo.sqlMess.yearlysalessql;
  this.$store.dispatch("psOverview/GetYearlySalesData").then(res => {
...
let creater = this.$store.state.user.userid;
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 13);
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr18: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
