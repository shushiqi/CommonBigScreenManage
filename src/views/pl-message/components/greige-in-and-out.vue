  ...
  <div class="container">
    <sp-card title="当日坯布出入库统计">
      ...
      <el-table v-if="switchBtn" :data="tableData">
        <el-table-column prop="grename" label="坯布名称" width="120">
        </el-table-column>
    ...
    >
      <el-form :model="tableTemp" ref="form" :rules="rules">
        <el-form-item label="坯布名称" label-width="120" prop="grename">
          <el-input
...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.daygreinandout;
    this.sqlString = this.$store.state.getInfo.sqlMess.daygreinandoutsql;
    this.$store.dispatch("plMessage/GetDayGreInAndOutData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr22: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
