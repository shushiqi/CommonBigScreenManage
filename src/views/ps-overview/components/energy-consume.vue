<template>
  <div class="container">
    <sp-card title="年度能耗统计" style="width: 40rem">
      <span slot="header">
      ...
      >
        ...
        <el-table-column prop="waterqty" label="水（吨）" width="120">
        </el-table-column>
    ...
    >
      <el-form :model="tableTemp" ref="form" :rules="rules">
        ...
        <el-form-item label="水（吨）" label-width="120" prop="waterqty">
          <el-input-number v-model="tableTemp.waterqty"></el-input-number>
        </el-form-item>
        <el-form-item label="电（度）" label-width="120" prop="elecqty">
          <el-input-number v-model="tableTemp.elecqty"></el-input-number>
        </el-form-item>
        <el-form-item label="蒸汽（千克）" label-width="120" prop="steamqty">
          <el-input-number v-model="tableTemp.steamqty"></el-input-number>
        </el-form-item>
      </el-form>
...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.energyconsume;
    this.sqlString = this.$store.state.getInfo.sqlMess.energyconsumesql;
    this.$store.dispatch("psOverview/GetEnergyConsumeData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 14);
        ...
        for (let index = 0; index < table.length; index++) {
          ...
          let item = {
            ...
            ep_attr2: "" + element.waterqty,
            ep_attr3: "" + element.elecqty,
            ep_attr4: "" + element.steamqty,
            createdby: creater,
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
    ...
    edit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.tableData[
            this.tableTemp.index
          ].waterqty = this.tableTemp.waterqty;
          this.tableData[this.tableTemp.index].elecqty = this.tableTemp.elecqty;
          this.tableData[
            this.tableTemp.index
          ].steamqty = this.tableTemp.steamqty;
