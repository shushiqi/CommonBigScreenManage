...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.mixingqueen;
    this.sqlString = this.$store.state.getInfo.sqlMess.mixingqueensql;
    this.$store.dispatch("plMessage/GetMixingQueenData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 6);
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr25: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
