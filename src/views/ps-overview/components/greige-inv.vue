<template>
  <div class="container">
    <sp-card title="当前坯布库存品种" style="width: 26.25rem">
      <span slot="header">
...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.greInv;
    this.sqlString = this.$store.state.getInfo.sqlMess.greInvsql;
    this.$store.dispatch("psOverview/GetGreInvData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 8);
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr13: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
