<template>
  <div class="container">
    <sp-card title="本年销售品种排行" style="width: 26.25rem">
      <span slot="header">
      ...
      <el-table v-if="switchBtn" :data="tableData">
        <el-table-column prop="fgname" label="品种" width="120">
        </el-table-column>
    ...
    >
      <el-form :model="tableTemp" ref="form" :rules="rules">
        <el-form-item label="品种" label-width="120" prop="fgname">
          <el-input
...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.yearlysalesrank;
    this.sqlString = this.$store.state.getInfo.sqlMess.yearlysalesranksql;
    this.$store.dispatch("psOverview/GetYearlySalesRankData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 10);
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr16: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
