<template>
  <div class="container">
    <sp-card title="本年投坯统计" style="width: 26.25rem">
      <span slot="header">
...
export default {
  ...
  mounted() {
    this.switchBtn = this.$store.state.getInfo.customizedData.yearlygrecasting;
    this.sqlString = this.$store.state.getInfo.sqlMess.yearlygrecastingsql;
    this.$store.dispatch("psOverview/GetYearlyGreCastingData").then(res => {
  ...
  methods: {
    save() {
      ...
      if (this.switchBtn) {
        let table = pushType(this.tableData, 12);
        ...
        postObj.push({
          ep_sqlsettingid: "",
          ep_attr17: replaceSQLString(this.sqlString),
          createdby: creater,
          createdon: createTime
        });
      }
